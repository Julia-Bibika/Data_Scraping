...
class ShevchenkoXpathSpider(scrapy.Spider):
    ...
    start_urls = ["http://www.univ.kiev.ua/"]

    def parse(self, response):
        fac_list = response.xpath('//ul[contains(@class,"b-references__holder")]').xpath('.//*[contains(@class,"b-references__item")]')
    ...
    def parse_faculty(self, response):
                dep_list = response.xpath('ol').css('.b-body__text')
                for li in dep_list:
